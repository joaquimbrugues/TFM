...
\begin{coro}
The set $\mathcal{L}(a,b)$ is finite when $\text{Ind}(a)-\text{Ind}(b) = 1$.
\end{coro}
...
\begin{theo}
If $a, b \in \crit(f)$ and $\text{Ind}(a)-\text{Ind}(b) = 2$, then $\overline{\mathcal{L}}(a,b)$ is a compact manifold with boundary.
\end{theo}

If we suppose this to be true, by the theorem \ref{1dimensional} we have that $\overline{\mathcal{L}}(a,b)$ must be diffeomorphic either to $\con{S}^1$ or to $[0,1]$. Furthermore, the set
...
As we just said, the term on the right is the boundary of a 1-dimensional compact manifold with boundary, so it either has 0 or 2 points. Therefore, its cardinal modulo 2 must be 0.
\end{proof}
