...
This vector field can be intrinsecally defined on a manifold $M$ using a Riemannian metric $g$ as the only vector field that satisfies that, for any vector field $Y \in \mathfrak{X}(M)$,
\[g(\grad f, Y) = d f \cdot Y .\]
...
\begin{enumerate}
\item For every $p \in M$ we have $(d f)_p \cdot X_p \leq 0$, and the equality holds if and only if $x$ is a critical point for $f$.
...
\begin{proof}
Let $c_1,...,c_r$ be the critical points of $f$ (there are finitely many as we saw in remark \ref{rmrk:finitecritial}), and let $(U_1,h_1),...,(U_r,h_r)$ be Morse charts in the neighbourhoods of these points. We assume that the images of these charts, which we will denote $\Omega_1,...,\Omega_r$, are disjoint. We can add more charts to get a finite open cover of $M$, $\{\Omega_j\}_{1 \leq j \leq m}$. We can also refine the open cover so that each critical point $c_i$ is contained only in its associated open set $\Omega_i$.

For each $\Omega_i$, let $X_i$ be the push-forward of the negative gradient of $\tilde{f}$ in $U_i$ by $h_i$. Also, take a partition of unity associated to $\{\Omega_j\}_{1 \leq j \leq m}$, $\{\varphi_j\}_{1 \leq j \leq m}$. We will have that $\varphi_i(c_i) = 1$ for all $i$ because of the refinement we just applied to the open cover of $M$. Then, we can define the vector fields
...
\[W^s(a) = \left\{ x \in M \ \middle| \ \lim_{s \rightarrow + \infty} \varphi^s(x) = a \right\}, \]
and its {\bf unstable manifold} as
\[W^u(a) = \left\{ x \in M \ \middle| \ \lim_{s \rightarrow - \infty} \varphi^s(x) = a \right\} .\]
\end{deff}
...
\end{displaymath}
This leads to the conclusion that $\displaystyle\lim_{s \rightarrow +\infty} f(\gamma(s)) = - \infty$, which is impossible since $M$ is compact.
\end{proof}

\subsection{The Smale condition}

The notion of connecting critical points between them needs to be complemented by the fact that the intersection of the unstable and stable manifolds is good enough, in the sense that it is regular. In particular, the notion that we need is the one of transversality.
...
\begin{deff}
Let $\mathcal{L}(a,b)$ denote the {\bf space of trajectories} between $a$ and $b$ defined as
\begin{displaymath}
...
\begin{coro}
If $\mathrm{Ind}(a)-\mathrm{Ind}(b) = 1$, then the set $\mathcal{L}(a,b)$ is discrete.
\end{coro}
...
\[\widetilde{X} = \sum_{i=1}^n a_i \frac{\partial}{\partial x_i} ,\]
...
\begin{proof}
First of all, construct a pair $(f,X)$ of Morse function and incoming pseudogradient adapted to $f$ as indicated in lemma \ref{incomingadapted}. As $M$ is 1-dimensional, the critical points of $f$ can only be local minima or maxima. Let $c_1,...,c_k$ be the minima of $f$.
